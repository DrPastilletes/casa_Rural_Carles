...
public class Finestra extends JFrame {
    ...
    public Logica c;
    public Fitxer gestorFitxers = new Fitxer();
    public String[] nomsFitxers = {"Client", "Habitacio", "ReservesPendents", "ReservesConfirmades"};
    ...
    private void iniciarFitxers() {
        gestorFitxers.creaDirectori();
        for (String nom : nomsFitxers) {
            ArrayList<String> contingutFitxer = gestorFitxers.llegirFitxer(gestorFitxers.crearFitxer(nom));
            switch (nom){
                case "Client":
                    c.hotel.crearClientFitxer(contingutFitxer);
                    break;
                case "Habitacio":
                    c.hotel.crearHabitacioFitxer(contingutFitxer);
                    break;
                case "ReservesPendents":
                    c.hotel.crearReservaPendent(contingutFitxer);
                    break;
                case "ReservesConfirmades":
                    c.hotel.crearReservaConfirmada(contingutFitxer);
                    break;
            }
        }
        System.out.println(c.hotel.getLlistaClients());
    }

    private void afegirContinguts(){
        for (Reserva res : c.hotel.getLlistaReservesPendents()) {
            if(res.arrayReservaPendent().length!=0){
                modelPendents.addRow(res.arrayReservaPendent());
            }
        }
        for (Reserva res : c.hotel.getLlistaReservesConfirmades()) {
            if(res.arrayReservaConfirmada().length!=0){
                modelPendents.addRow(res.arrayReservaConfirmada());
            }
        }
    }
...
//					c.afegirReservaHotel(resAux);
                    ...
                    if (resAux != null) {
                        ...
                        reserva.setEnabled(false);
                        gestorFitxers.escriureAFitxer(c.hotel.castClients(),"Client");
                        gestorFitxers.escriureAFitxer(c.hotel.castReserves(c.hotel.getLlistaReservesPendents()),"ReservesPendents");
                    } else {
                        String[] options2 = {"Ok"};
    ...
    private void afegirActionListenerHabitacio() {
        ActionListener clickHabitacio = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                gestorFitxers.escriureAFitxer(c.hotel.castHabitacions(), "Habitacio");
            }
        };
        ...
        taulaReservesP.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (e.getClickCount() == 2) {
                    ...
                    gestorFitxers.escriureAFitxer(c.hotel.castReserves(c.hotel.getLlistaReservesPendents()),"ReservesPendents");
                    gestorFitxers.escriureAFitxer(c.hotel.castReserves(c.hotel.getLlistaReservesConfirmades()),"ReservesConfirmades");
                }
            }
        });
    }
